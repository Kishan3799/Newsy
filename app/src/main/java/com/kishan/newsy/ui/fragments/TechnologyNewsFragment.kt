...
class TechnologyNewsFragment : Fragment(R.layout.technology_news_fragment) {
    ...
    lateinit var technologyRV: RecyclerView
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        progressCircularBar = view.findViewById(R.id.progress_circular_tech)
        technologyRV = view.findViewById(R.id.techRv)
        ...
        mainViewModel.technologyNews.observe(viewLifecycleOwner) { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let { newsResponse ->
                        ...
                        isLastPage = mainViewModel.technologyNewsPage == totalPage
                        if (isLastPage) {
                            technologyRV.setPadding(0, 0, 0, 0)
                        }
                    }
                }

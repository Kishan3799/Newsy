...
class EntertainmentNewsFragment : Fragment(R.layout.entertainment_news_fragment) {
    lateinit var mainViewModel: MainViewModel
    lateinit var newsAdapter: NewsAdapter
    lateinit var entertainRv: RecyclerView
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        progressCircularBar = view.findViewById(R.id.progress_circular_ent)
        entertainRv = view.findViewById(R.id.entertainmentRv)
        ...
        mainViewModel.entertainmentNews.observe(viewLifecycleOwner) { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let { newsResponse ->
                        ...
                        isLastPage = mainViewModel.entertainmentNewsPage == totalPage
                        if (isLastPage) {
                            entertainRv.setPadding(0, 0, 0, 0)
                        }
                    }
                }

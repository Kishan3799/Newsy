...
class SportsNewsFragment : Fragment(R.layout.sports_news_fragment) {
    lateinit var mainViewModel: MainViewModel
    lateinit var newsAdapter: NewsAdapter
    lateinit var sportsRV: RecyclerView
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        progressCircularBar = view.findViewById(R.id.progress_circular_spo)
        ...
        mainViewModel.sportsNews.observe(viewLifecycleOwner) { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let { newsResponse ->
                        ...
                        isLastPage = mainViewModel.sportsNewsPage == totalPage
                        if (isLastPage) {
                            sportsRV.setPadding(0, 0, 0, 0)
                        }
                    }
                }
